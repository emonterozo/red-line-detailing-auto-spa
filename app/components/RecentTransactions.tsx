...
const RecentTransactions = () => {
  ...
  useEffect(() => {
    const init = async () => {
      setIsLoading(true);
      getTransactions(page, PAGE_LIMIT).then(
  ...
  return (
    <section className="mb-8 rounded-2xl border border-white/[0.08] bg-white/[0.02] backdrop-blur-sm overflow-hidden custom-scrollbar">
      ...
      {isLoading ? (
        <TableSkeleton tableHeader={TABLE_HEADER} />
      ) : (
